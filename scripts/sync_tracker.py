...
Credentials:
    ...
    .env                TRACKER_SHEET_ID (or TRACKER_SHEET_TITLE) and
                        one *_BUFFER_API_KEY per Buffer account
...
# is why the id is the primary route. Never hardcode a project title here.
SHEET_ID_VAR = "TRACKER_SHEET_ID"
SHEET_TITLE_VAR = "TRACKER_SHEET_TITLE"
...
def open_spreadsheet(creds, env: dict[str, str]) -> str:
    ...
    sheet_title = env.get(SHEET_TITLE_VAR) or os.environ.get(SHEET_TITLE_VAR)
    if not sheet_title:
        raise SystemExit(
            f"Cannot locate the tracker sheet. Add {SHEET_ID_VAR}=<id> to .env "
            "(the id is the long string in the spreadsheet URL). As a fallback, "
            f"set {SHEET_TITLE_VAR} to the Drive file name and share the sheet "
            "directly with the client_email in keys/service.json."
        )
    ...
    if not found:
        raise SystemExit(
            f"Cannot locate a Drive spreadsheet named {sheet_title!r}. "
            f"Add {SHEET_ID_VAR}=<id> to .env, or share a sheet with that "
            "title with the client_email in keys/service.json."
        )
    return found[0]["id"]
