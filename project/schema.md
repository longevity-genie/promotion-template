...
**On `status`:** prefer `retired` over deleting a row. A room that's wrong today may suit a later pillar, and the rule research stays useful either way. `comment-only` means participate but never post — the honest state for several science subreddits. `on-hold` means we are not ready to post yet (missing asset, print photo, STL, etc.) — not that moderators banned us. Put the release condition in `notes`.
